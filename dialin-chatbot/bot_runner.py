...
async def _create_daily_room(room_url, callId, callDomain=None, vendor="daily"):
    if not room_url:
        ...
        try:
            ...
            raise HTTPException(
                status_code=504, detail=f"Room not found: {room_url}. Error: {str(e)}")
    ...
    if not room or not token:
        raise HTTPException(
            status_code=503, detail=f"Failed to get room or token token")
    ...
    try:
        ...
        raise HTTPException(
            status_code=502, detail=f"Failed to start subprocess: {e}")
...
async def twilio_start_bot(request: Request):
    ...
    parsed_data = parse_qs(form_data.decode('utf-8'))
    
    # Extract CallSid from the parsed data
    callId = parsed_data.get('CallSid', [None])[0]

    room_url = os.getenv("DAILY_SAMPLE_ROOM_URL", None)

    if not callId:
        raise HTTPException(
            status_code=501, detail="Missing 'CallSid' in request")
