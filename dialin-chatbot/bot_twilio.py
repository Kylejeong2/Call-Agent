...
async def main(room_url: str, token: str, callId: str, sipUri: str):
    async with aiohttp.ClientSession() as session:
        ...
        async def on_dialin_ready(transport, cdata):
            logger.info(f"on_dialin_ready event fired. Forwarding call: {callId} {sipUri}")

            try:
                call = twilioclient.calls(callId).update(
                    twiml=f'<Response><Dial><Sip>{sipUri}</Sip></Dial></Response>'
                )
                logger.info(f"Call forwarded successfully. Call SID: {call.sid}")
            except Exception as e:
                logger.error(f"Failed to forward call: {str(e)}")
                # Optionally, you can re-raise the exception if you want to stop the execution
                # raise
